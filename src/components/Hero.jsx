...
export function Hero(){
    ...
    const options = {
        ...
        headers: {
          ...
          Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIzZGJhMDgzODlkOWM5NzdiOTRjM2RiMmQxZDRlNWYzOCIsInN1YiI6IjY0ZmU0NTAxZmZjOWRlMGVlM2M0YzJlZiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.z-rZK7imtrGnlWaGdNJUDrtD6UtyOfZG_CGGeaQ3pF8'
        }
      };
    ...
    return(
        ...
        {trending.results !== undefined && trending.results.slice(indexs-1,indexs).map((movies,index) => {
            return(
        <section key = {index} className={`font-DM-sans px-4 md:px-12 lg:px-20 min-h-screen sm:px-8 bg-cover bg-no-repeat bg-top `} style={{backgroundImage : `url(https://www.themoviedb.org/t/p/original/${movies.backdrop_path})`}}>
            ...
            <form className="w-full mt-4 xs:hidden flex pl-3 py-1 rounded-md border-solid border-gray-300 border-2 text-white">
                    <input type="text" className="bg-transparent w-full outline-none placeholder:text-white" placeholder="What do you want to watch?" autoComplete="off" spellCheck = "false"/>
                    <img src="/assets/images/search.svg" alt="search icon" className="mx-4"/>
            </form>
